...
if (!baselinePath || !currentPath) {
  console.error("Usage: node compare-patch-proposal-reports.mjs --baseline <baseline.json> --current <current.json> [--output <comparison.json>]");
  process.exit(2);
}
...
compareSummaryDecrease("patchValidity");
compareSummaryDecrease("safetyPassedCases");
compareSummaryDecrease("rollbackReadyCases");
compareSummaryDecrease("evidenceLinkedCases");
compareSummaryDecrease("testCommandReadyCases");
...
for (const [caseId, baselineCase] of baselineCases.entries()) {
  ...
  compareCaseNumber(caseId, "patchValidity.score", baselineCase.patchValidity?.score, currentCase.patchValidity?.score);
  compareCaseBoolean(caseId, "safety.passed", baselineCase.safety?.passed, currentCase.safety?.passed);
  compareCaseBoolean(caseId, "rollback.passed", baselineCase.rollback?.passed, currentCase.rollback?.passed);
  compareCaseBoolean(caseId, "evidence.passed", baselineCase.evidence?.passed, currentCase.evidence?.passed);
  compareCaseBoolean(caseId, "testCommands.passed", baselineCase.testCommands?.passed, currentCase.testCommands?.passed);
}

const comparison = {
  schema: "learnbot.quality.patch-proposal-comparison.v1",
...
console.log(`patch proposal comparison passed: ${currentPath}`);
...
function validateReport(label, report) {
  if (report?.schema !== "learnbot.quality.patch-proposal-score.v1") {
...
function compareCaseNumber(caseId, key, baselineValue, currentValue) {
  if (baselineValue === undefined || currentValue === undefined) {
    return;
  }
  const baselineNumber = Number(baselineValue);
  const currentNumber = Number(currentValue);
  if (Number.isFinite(baselineNumber) && Number.isFinite(currentNumber) && currentNumber < baselineNumber) {
    regressions.push(`case-metric-decreased:${caseId}:${key}:${baselineNumber}->${currentNumber}`);
  }
}
