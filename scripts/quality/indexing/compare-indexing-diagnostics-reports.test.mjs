...
const scriptPath = path.join(scriptDir, "compare-indexing-diagnostics-reports.mjs");
const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "learnbot-indexing-comparison-"));
...
const baselineReport = {
  ...
  summary: {
    ...
    passedCases: 1,
    failedCases: 0,
    searchableFallbackPassedCases: 1,
    retryReadyCases: 1,
  ...
  results: [
    {
      id: "crawler-allowlist-budget-stores-source-pages",
      passed: true,
      searchableFallback: { passed: true },
      retryReadiness: { passed: true },
...
const regressedReport = {
  ...
  summary: {
    ...
    passedCases: 0,
    failedCases: 1,
    searchableFallbackPassedCases: 0,
    retryReadyCases: 0,
  ...
  results: [
    {
      id: "crawler-allowlist-budget-stores-source-pages",
      passed: false,
      searchableFallback: { passed: false },
      retryReadiness: { passed: false },
...
try {
  ...
  assert.match(passingResult.stdout, /indexing diagnostics comparison passed/);
  const passingComparison = JSON.parse(fs.readFileSync(comparisonPath, "utf8"));
  assert.equal(passingComparison.schema, "learnbot.quality.indexing-diagnostics-comparison.v1");
  ...
  assert.match(failingResult.stderr, /summary-decreased:passedCases/);
...
console.log("compare-indexing-diagnostics-reports tests passed");
