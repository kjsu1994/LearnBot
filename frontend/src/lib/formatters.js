...
function getCodeModeGuide(mode) {
  const guides = {
    ...
    reasoning: {
      title: '구현 의도 질문 예시',
      description: '코드가 왜 이렇게 구성됐는지, 어떤 책임과 절충이 보이는지 근거와 추정을 분리해 설명받을 때 사용합니다.',
      placeholder: '예: 이 로직은 왜 서비스에서 처리하고 컨트롤러에서는 검증만 해?',
      tips: [
        '“왜”, “의도”, “설계상 이유”, “이 구현 괜찮아?”처럼 물어보면 자동으로 이 관점에 가깝게 분류됩니다.',
        '근거 코드와 추정 영역을 나눠 답하므로, 실제 설계 문서가 없을 때도 검토 지점이 명확해집니다.',
        '변경까지 고민 중이면 “수정하면 영향받는 곳”을 함께 적으세요.',
      ],
    },
    locate: {
      title: '위치 찾기 질문 예시',
