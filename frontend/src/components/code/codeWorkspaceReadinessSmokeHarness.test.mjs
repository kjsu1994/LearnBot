...
assert.equal(props.codeAgentLoopTimelines[0].events[11].eventType, 'STOP_OUTCOME_RECORDED');
assert.equal(props.codeAgentLoopTimelines[0].events[11].details.stopKey, 'TOOL_FAILED');
assert.equal(props.codeAgentLoopTimelines[0].events[11].mayMutate, false);
assert.equal(props.codeAgentLoopTimelines[0].events[12].eventType, 'STOP_OUTCOME_RECORDED');
assert.equal(props.codeAgentLoopTimelines[0].events[12].details.stopKey, 'APPROVAL_DENIED');
assert.equal(props.codeAgentLoopTimelines[0].events[12].mayMutate, false);
assert.equal(props.codeAgentLoopTimelines[0].events[13].eventType, 'STOP_OUTCOME_RECORDED');
assert.equal(props.codeAgentLoopTimelines[0].events[13].details.stopKey, 'AGENT_UNAVAILABLE');
assert.equal(props.codeAgentLoopTimelines[0].events[13].mayMutate, false);
assert.equal(props.codeAgentLoopTimelines[0].events[14].eventType, 'STOP_OUTCOME_RECORDED');
assert.equal(props.codeAgentLoopTimelines[0].events[14].details.stopKey, 'TIMEOUT');
assert.equal(props.codeAgentLoopTimelines[0].events[14].mayMutate, false);
assert.equal(props.codeAgentLoopTimelines[0].events[15].eventType, 'STOP_OUTCOME_RECORDED');
assert.equal(props.codeAgentLoopTimelines[0].events[15].details.stopKey, 'CANCELLATION');
assert.equal(props.codeAgentLoopTimelines[0].events[15].mayMutate, false);
assert.equal(props.codeAgentLoopRunnerEnqueueResult.runnerDecision, 'NOT_ENQUEUED');
assert.equal(props.codeAgentLoopRunnerEnqueueResult.handoffSummary.status, 'READY_HANDOFF_CREATION_DISABLED');
assert.equal(props.codeAgentLoopRunnerEnqueueResult.handoffSummary.expectedRequestCount, 4);
assert.equal(props.codeAgentLoopRunnerEnqueueResult.handoffSummary.durableMutationExecutionRowCount, 0);
assert.equal(props.codeAgentLoopRunnerEnqueueResult.requestCreationEnabled, false);
assert.equal(props.codeAgentLoopRunnerEnqueueResult.enqueueEnabled, false);
assert.equal(props.codeAgentLoopRunnerEnqueueResult.pushEnabled, false);
assert.equal(props.codeAgentLoopRunnerEnqueueResult.claimEnabled, false);
assert.equal(props.codeAgentLoopRunnerEnqueueResult.mutationEnabled, false);
